...
namespace CM.API.Controllers
{
    ...
    public class CypherController : ControllerBase
    {
        ...
        public CypherController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        public async Task<IActionResult> Import(ImportCoinBlock input)
        {
            try
            {
                var result = await _mediator.Send(new ImportCypherBlockCommand(input.IsTest));
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetHistory([FromQuery] bool isTest, [FromQuery] short limit)
        {
            try
            {
                var result = await _mediator.Send(new GetCypherBlocksQuery(isTest, limit));
                return Ok(result);
            }
            catch (Exception ex)
            {
